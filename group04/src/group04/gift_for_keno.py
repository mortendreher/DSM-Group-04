...
def write_graph_to_csv():
    ...
    visitednodes = {granddad_id}  # visited nodes saved here (as key without value)
    with open('graph_keno.csv', 'w', newline='') as csvfile:
        ...
        killswitch = 500000  # -> max number of rows written before lowest plane
        while len(children) > 0 and killswitch > 0:
            children_todo = []
            for child in children:
                if child not in visitednodes:  # child has not been visited yet
                    children_row = get_node_row(child)
                    graphwriter.writerow({child + children_row})
                    visitednodes.update(child)  # set child as visited
                    ...
                    children.remove(child)  # remove child from list of nodes to be visited
